"""Peer quality for Cost Intelligence V1.1.

Quality is not a function of raw peer count alone. A large group of loosely
related Normal/Others titles scores lower than a small, title-similar group.
"""
...
from app.engines.cost.types import PeerQuality, PeerRecord, PeerScope
from app.engines.cost.work_type import work_type_similarity


def _similarities(subject: PeerRecord, peers: Sequence[PeerRecord]) -> list[float]:
    return [
...
def compute_peer_quality(
    ...
    if not peers or scope is None:
        return PeerQuality(
            score=0,
            rationale=(
                "No sufficient peer group was selected, so peer quality is 0."
            ),
            median_work_type_similarity=0.0,
            category_match_rate=0.0,
        )
    sims = _similarities(subject, peers)
